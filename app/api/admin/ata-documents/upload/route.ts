...
import { ataDocLibUpload, isAtaDocLibConfigured } from "@/lib/ata-doc-library";
...
/**
 * POST /api/admin/ata-documents/upload
...
export async function POST(req: NextRequest) {
  ...
  const { ok, status, data } = await ataDocLibUpload(form);
  if (!ok) {
    return NextResponse.json({ error: (data as { error?: string })?.error || "上传失败" }, { status: status || 502 });
  }
