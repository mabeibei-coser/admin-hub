  ...
  const submit = async () => {
    ...
    try {
      ...
      if (file) {
        ...
        const upRes = await fetch(withBase("/api/admin/ata-documents/upload"), {
          method: "POST",
          credentials: "include",
          body: fd,
        });
      ...
      const url = isEdit
        ? withBase(`/api/admin/ata-documents/${editing!.id}`)
        : withBase("/api/admin/ata-documents");
...
function AtaDocumentsInner() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(withBase(`/api/admin/ata-documents?${params}`), {
        credentials: "include",
      });
  ...
  const doDelete = async () => {
    ...
    try {
      const res = await fetch(withBase(`/api/admin/ata-documents/${deleteId}`), {
        method: "DELETE",
        credentials: "include",
      });
      if (!res.ok) {
  ...
  return (
    <div className="relative p-6">
      ...
      {error && <Alert tone="error">{error}</Alert>}
      {!configured && !loading && (
        <Alert tone="warning">文档库未配置（ATA_DOC_LIB_SECRET 缺失），无法管理文档</Alert>
      )}
...
export default function AtaDocumentsPage() {
  return (
