...
const TYPE_META: Record<AgreementType, { label: string; icon: typeof FileText; hint: string }> = {
  terms: {
    label: "服务使用协议",
    icon: ScrollText,
    hint: "用户在登录页勾选「我已阅读并同意」后才能登录；此处编辑的内容会被前台用户在协议页查看到。",
  },
  privacy: {
    label: "隐私政策",
    icon: ShieldCheck,
...
export default function AtaAgreementsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(withBase("/api/admin/ata-agreements"));
      if (!res.ok) {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        icon={FileText}
        title="ATA 协议管理"
        subtitle="编辑薪资查询用户在登录页看到的服务使用协议与隐私政策（Markdown 格式，保存即生效）"
        actions={
      ...
      {!ready && (
        <Alert tone="warning" title="ATA 数据库未就绪">
          ATA100 数据库（ata100.db）尚未生成或未挂载。请先启动 ATA100 服务初始化数据库后再回来编辑。
        </Alert>
      )}
