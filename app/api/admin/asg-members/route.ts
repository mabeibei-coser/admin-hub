...
import { getAdminDb, isAsgDbReady, isHazardDbReady } from "@/lib/db";
...
export async function GET(req: NextRequest) {
  ...
  // 使用次数 = 该手机号在 hazard 域（隐患识别）生成的报告数。
  // hazard 库可能未 ready（dev 没起过 A600 / 生产首次部署前 ATTACH 空文件）
  // → 表不存在，prepare 直接报错。所以入口判断后再 inline SQL。
  const usageExpr = isHazardDbReady()
    ? "(SELECT COUNT(*) FROM hazard.reports r WHERE r.user_phone = m.phone)"
    : "0";
  const rows = db
    .prepare(
      `SELECT
