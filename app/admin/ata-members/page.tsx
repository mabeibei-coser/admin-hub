...
import { AtaMemberDetailDialog } from "@/components/admin/ata-member-detail-dialog";
...
function AtaMembersInner() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(withBase(`/api/admin/ata-members?${params}`), { credentials: "include" });
  ...
  return (
    <div className="relative p-6">
      ...
      {!ready && !loading && (
        <Alert tone="warning">会员数据库暂不可用（ata100 可能未部署或尚无会员）</Alert>
      )}
...
export default function AtaMembersPage() {
  return (
