 ...
 * - 充值记录：GET /api/admin/ata-members/[phone] → orders 按 created_at DESC
 * - 仅超管显示「编辑 VIP 到期日 / 撤销 VIP」面板。改动经 PATCH 路由，
 *   ata.memberships UPDATE + ata.membership_ledger(type='admin_adjust') 同事务写入。
...
export function AtaMemberDetailDialog({
  phone,
  isSuper,
  onClose,
  onSaved,
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        withBase(`/api/admin/ata-members/${encodeURIComponent(phone)}`),
        { credentials: "include" }
      );
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="ata-detail-title"
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border px-5 py-3.5 sticky top-0 bg-card z-10">
          <div className="flex items-center gap-2.5">
            ...
            <h2 id="ata-detail-title" className="text-base font-semibold text-foreground">
              会员详情
            </h2>
              ...
              {isSuper && (
                <section className="rounded-lg border border-border bg-muted/30 px-4 py-3">
                  ...
                  <p className="mt-2 text-[11px] text-muted-foreground leading-relaxed">
                    ...
                    撤销 VIP = 把到期日清零。所有修改在 ata.membership_ledger 留下审计记录。
                  </p>
                </section>
              )}
            </>
