...
/** PUT /api/admin/ata-documents/[id] — 编辑文档（转发 doc-library）。 */
export async function PUT(
  ...
  const { ok, status, data } = await ataDocLibFetch(`/api/admin/documents/${id}`, { method: "PUT", body });
  if (!ok) {
...
/** DELETE /api/admin/ata-documents/[id] — 删除文档（转发 doc-library）。 */
export async function DELETE(
  ...
  const { id } = await params;
  const { ok, status, data } = await ataDocLibFetch(`/api/admin/documents/${id}`, { method: "DELETE" });
  if (!ok) {
