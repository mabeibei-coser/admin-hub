...
/**
 * GET /api/admin/asg-members/[phone]
...
/**
 * PATCH /api/admin/asg-members/[phone]
 * 超管手动调整 VIP 到期时间（运营 / 退款 / 客诉补偿）。
 *
 * 破例说明：admin-hub 默认只读 asg.* 库。经用户授权，本接口可写
 *   - asg.memberships.vip_expire_at / updated_at
 *   - asg.membership_ledger（追加一条 type='admin_adjust' 流水做审计）
...
export async function PATCH(
  ...
  const cur = db
    .prepare("SELECT vip_expire_at FROM asg.memberships WHERE phone = ?")
  ...
  const tx = db.transaction(() => {
    db.prepare(
      `UPDATE asg.memberships SET vip_expire_at = ?, updated_at = ? WHERE phone = ?`
    ).run(newExpireAt, now, phone);
    db.prepare(
      `INSERT INTO asg.membership_ledger
